...
import { MenuSelectors } from 'Lib/Store/menu'
import { UserSelectors } from 'Lib/Store/user'
import style from './style.module.scss'
import './style.sass'

export const MenuLeft = () => {
  const { menuData } = useSelector(MenuSelectors.menu)
  const isMenuCollapsed = useSelector(SettingsSelectors.isMenuCollapsed)
  const isMobileView = useSelector(SettingsSelectors.isMobileView)
  const isMenuUnfixed = useSelector(SettingsSelectors.isMenuUnfixed)
  const isMenuShadow = useSelector(SettingsSelectors.isMenuShadow)
  const leftMenuWidth = useSelector(SettingsSelectors.leftMenuWidth)
  const menuColor = useSelector(SettingsSelectors.menuColor)
      ...
      >
        <div className={style.logoContainer}>
          <div className={style.logo}>
            <img src="resources/images/logo.svg" className="mr-2 logo-size" alt="CryptoPlatform" />
            <div className="platform-name">Cryptoplatform</div>
          </div>
        </div>
        <PerfectScrollbar>
          <Menu
